...
async def get_great_quality_stocks():
    ...
    try:
        ...
        for stock in stocks:
            try:
                stock_responses.append(_stock_to_response(stock))
            except Exception as e:
                print(f"Error converting stock {stock.nse_code} to response: {e}")
                continue
        ...
        import traceback
        error_detail = f"Error fetching great quality stocks: {str(e)}\n{traceback.format_exc()}"
        print(error_detail)
...
async def get_debug_info():
    """
    Debug endpoint: always returns 200 with excel_path, excel_file_exists, and load stats.
    Use this to verify whether the data file is present on the server.
    """
    excel_path = getattr(quality_service, "excel_path", None) or "Unknown"
    excel_file_exists = os.path.exists(excel_path) if excel_path and excel_path != "Unknown" else False

    out = {
        "excel_path": excel_path,
        "excel_file_exists": excel_file_exists,
        "total_stocks_loaded": 0,
        "stocks_meeting_great_criteria": 0,
        "sample_stocks": [],
        "note": "If excel_file_exists is false, add data/trendlyne_data.xlsx or set QUALITY_STOCKS_EXCEL_PATH.",
    }

    try:
        stocks = quality_service.load_stocks()
        out["total_stocks_loaded"] = len(stocks)
    except Exception as e:
    ...
    try:
        ...
        for stock in stocks[:10]:
            try:
                ...
                scored_stocks.append({
                    ...
                    "eps_ttm_growth": stock.eps_ttm_growth,
                    "operating_rev_growth_ttm": stock.operating_rev_growth_ttm,
                    "quality_score": stock.quality_score,
                })
            except Exception:
                continue
        out["sample_stocks"] = scored_stocks
        ...
        for stock in stocks:
            try:
                ...
                continue
        out["stocks_meeting_great_criteria"] = great_count
    except Exception as e:
