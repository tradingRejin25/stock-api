...
**Path resolution (order tried):**  
1. `QUALITY_STOCKS_EXCEL_PATH` env var (if set and file exists).  
2. **Data next to app root:** `data/trendlyne_data.xlsx` relative to the directory that contains `services/` (i.e. the same root as `main.py`). So if your repo root is the app (e.g. `main.py` and `data/` at root), the file must be at **`data/trendlyne_data.xlsx`** in that root.  
3. `os.getcwd()/data/trendlyne_data.xlsx`.  
4. `os.getcwd()/stock_api_service/data/trendlyne_data.xlsx` (when cwd is repo root and file is inside `stock_api_service`).  

If you see `ERROR: Excel file not found at: /opt/render/project/src/data/trendlyne_data.xlsx`, that path is either (1) the app root plus `data/` or (2) the current working directory plus `data/`. Put `trendlyne_data.xlsx` in the **`data/`** folder at the **root of the code that runs** (where `main.py` lives), and ensure the file is **committed to the repo** and not listed in `.gitignore`.
